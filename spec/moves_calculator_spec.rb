...
describe MovesCalculator do
  ...
  context 'pawns movements' do
    ...
    it 'returns 1 pawns\'s move from a6 if there is a piece in b7' do
      ...
      expect(subject.legal_moves([2, 0])).to match_array([[1, 1]])
    end
    context 'the pawn moves along the A column' do
      it 'returns 1 up move from a6 if there is not piece at a7, then follow the path' do
        ...
        expect(subject.legal_moves([2, 0])).to match_array([[1, 0], [1, 1]])
        moves_controller.make_move([2, 0], [1, 0], player_white)
        subject.board.delete_piece([0, 1])
        expect(subject.legal_moves([1, 0])).to be_empty
      end
    end
    ...
    context 'the pawn moves along the A column' do
      it 'returns 1 down move from a3 if there is not piece at a2, then follow the path' do
        ...
        expect(subject.legal_moves([5, 0])).to match_array([[6, 0], [6, 1]])
        moves_controller.make_move([5, 0], [6, 0], player_black)
        subject.board.delete_piece([7, 1])
        expect(subject.legal_moves([6, 0])).to be_empty
      end
    end
  ...
  context 'modified boards' do
    ...
    describe 'black king can castle short if there aren\'t knight and bishop ' do
      it 'returns array with short castle move included [0, 6]' do
        ...
        expect(subject.legal_moves([0, 4])).to match_array([[0, 5], [0, 6]])
      end
    end
    describe 'black king can castle long if there aren\'t queen, knight and bishop ' do
      it 'returns array with long castle move included [0, 2]' do
        ...
        expect(subject.legal_moves([0, 4])).to match_array([[0, 3], [0, 2]])
      end
    end
  ...
  context 'pinned pieces and legal moves' do
    ...
    describe 'knight is pinned' do
      it 'returns empty legal moves' do
        ...
        expect(subject.legal_moves([6, 4])).to be_empty
      end
    end
    describe 'bishop is pinned' do
      it 'returns empty moves because queen isnt pinning its diagonally' do
        ...
        expect(subject.legal_moves([5, 4])).to be_empty
      end
      it 'returns legal moves only towards queen pinning diagonally from h4' do
        ...
        expect(subject.legal_moves([6, 5])).to match_array([[5, 6], [4, 7]])
      end
    end
    describe 'queen is pinned' do
      it 'returns legal moves only towards pinner or king' do
        ...
        expect(subject.legal_moves([5, 4])).to match_array([[1, 4], [2, 4], [3, 4], [4, 4], [6, 4]])
      end
    end
  end
  context 'under check forced pieces ' do
    ...
    describe 'the king is in check, pinned pieces can not move ' do
      ...
      it 'the king cant move to xrayed squares by the checker' do
        8.times { |i| subject.board.delete_piece([6, i - 1]) } # delete white pawns 
        moves_controller.make_move([0, 3], [3, 4], player_black)
        moves_controller.make_move([0, 0], [7, 0], player_black)
        moves_controller.make_move([7, 3], [6, 4], player_white)
        subject.board.delete_piece([7, 1]) # delete knight
        subject.board.delete_piece([7, 2]) # delete bishop
        subject.board.delete_piece([7, 5]) # delete knight
        ...
        king = subject.board.king(true)
        checker = subject.checker(king)
        expect(checker.square).to match_array([7, 0])
        expect(subject.forced_pieces(king, checker).values.flatten(1)).not_to include([7, 5])
      end
      it 'under double check only king can move' do
        8.times { |i| subject.board.delete_piece([6, i - 1]) } # delete white pawns 
        moves_controller.make_move([0, 3], [3, 4], player_black)
